...
import VizQuestion from "@/components/experimentSix/VizQuestion";
...
export default function Experiment6() {
  ...
  useEffect(() => {
    ...
    if (user) {
      unsubscribe = onSnapshot(doc(db, "experiment_6", user.uid), (doc) => {
        if (doc.exists()) {
          setUserAlreadyAnswered(true);
        }
      });
      ...
      signInAnonymously(auth).then((user) => {
        unsubscribe = onSnapshot(
          doc(db, "experiment_6", user.user.uid),
          (doc) => {
            if (doc.exists()) {
              setUserAlreadyAnswered(true);
            }
  ...
  async function sendAnswer(frequency: number) {
    ...
    if (user) {
      ...
      const generalDocRef = doc(db, "experiment_6", "general");
